...
parser.add_argument(
    ...
    type=float,
    default=0.05,
    help="Leniency to use in order to select the threshold.",
)
...
output_dir = os.path.dirname(args.prediction_path) + f"/confidence_intervals/segmentation_prediction-l={args.leniency}"
...
for subject_id, _, target, pred in tqdm(IterIdExamTargetPred(args.prediction_path)):
    ...
    min_leniency_quantile = args.leniency
    if target.sum() != 0:
        min_of_prediction_inside_target = min(np.quantile(pred[target > 0.5], min_leniency_quantile), 0.5)
    else:
        min_of_prediction_inside_target = 0.5

    max_leniency_quantile = 1 - args.leniency * np.count_nonzero(target) / np.count_nonzero(1 - target)
    max_of_prediction_outside_target = max(np.quantile(pred[target < 0.5], max_leniency_quantile), 0.5)

    threshold_data = {
        "threshold_inflation": min_of_prediction_inside_target,
        "threshold_deflation": max_of_prediction_outside_target,
...
for confidence in args.confidences:
    alpha = 1 - confidence
    upper_bound_thresholds.append(np.quantile([x["threshold_inflation"] for x in per_subject_thresholds], alpha / 2))
    lower_bound_thresholds.append(
        np.quantile([x["threshold_deflation"] for x in per_subject_thresholds], 1 - alpha / 2)
    )
...
def save_intervals(prediction_path: str, output_dir: str, dataset_name: str) -> None:
    ...
    for subject_id, _, target, pred in tqdm(IterIdExamTargetPred(prediction_path)):
        for j in range(len(args.confidences)):
            ...
            interval_entry = {}
            interval_entry["upper_volume"] = volume_from_mask(upper_bound_mask, subject_id)
            interval_entry["lower_volume"] = volume_from_mask(lower_bound_mask, subject_id)
            interval_entry["prediction_volume"] = volume_from_mask((pred > 0.5), subject_id)
            interval_entry["target_volume"] = volume_from_mask(target, subject_id)
    ...
    for i, confidence in enumerate(args.confidences):
        ...
        with open(
            output_dir + f"/1d_intervals-segmentation_prediction-l={args.leniency}-c={confidence}.json", "w"
        ) as f:
